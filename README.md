...
| 模式 | 主要目标 | 翻译策略 | 媒体流水线 |
| --- | --- | --- | --- |
| Economy | 降低请求数和成本 | 使用较大的自包含 batch；结构失败先纠正一次再拆分；默认关闭全文术语预检、在线术语和模型审校 | 缓存转录块，达到配置的 batch 或 token 阈值后再翻译 |
| Turbo | 平衡延迟、吞吐与一致性 | 自适应并发翻译，使用相邻原文、受行数/token 限制的已确认译文和轻量人名/术语对齐 | 每个稳定的 10 分钟核心块完成后立即开始翻译 |
| Cinema | 优先保证全片质量与一致性 | 场景感知调度、严格全文术语预检、在线术语、时间轴感知完整审校和语言感知可读性限制 | 等待完整转录后再翻译，以便使用全文上下文 |
